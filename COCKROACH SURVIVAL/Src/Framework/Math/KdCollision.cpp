...
// 追加
// 取得した３頂点から面を作成し、法線ベクトルを取得する
static void calcNormal(CollisionMeshResult& result, const std::vector<Math::Vector3>& vertexPos)
{
	// 格納されている頂点数が３つかどうかを確認
	if (vertexPos.size() != 3)return;

	// 3頂点を取得
	const Math::Vector3& _pointA = vertexPos[0];
	const Math::Vector3& _pointB = vertexPos[1];
	const Math::Vector3& _pointC = vertexPos[2];

	// ３頂点から２つのベクトルを算出
	Math::Vector3 VecAB = _pointB - _pointA;
	Math::Vector3 VecAC = _pointC - _pointA;

	// ２つのベクトルの外積を計算して法線を算出
	Math::Vector3 _normal = VecAB.Cross(VecAC);

	// 法線ベクトルの正規化
	_normal.Normalize();

	// 結果を保存する
	result.m_normalVec = _normal;
}
	...
	float rayRange, const DirectX::XMMATRIX& matrix, CollisionMeshResult* pResult)
{
	...
	// 追加：
	// 一番近かった面の３頂点を保存しておくための変数
	std::vector<Math::Vector3> _vertexPos;
	...
	for (UINT faceIdx = 0; faceIdx < faceNum; ++faceIdx)
	{
		...
		if (hitDist < closestDist)
		{
			...
			// _vertexPosをクリアして新しい頂点座標を保存
			_vertexPos.clear();
			_vertexPos.push_back(vertices[idx[0]]);
			_vertexPos.push_back(vertices[idx[1]]);
			_vertexPos.push_back(vertices[idx[2]]);
		}
